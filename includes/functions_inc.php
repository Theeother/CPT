...
////////////////////////// check if solved and get answer

function get_ans($conn,$userId,$probId){
    $sql="SELECT * from solved where probId = ? and userId=?;";
        $stmt=mysqli_stmt_init($conn);
        mysqli_stmt_prepare($stmt,$sql);

        mysqli_stmt_bind_param($stmt, "ii",$probId,$userId);
        mysqli_stmt_execute($stmt);
        $resultData=mysqli_stmt_get_result($stmt);
        $ans=mysqli_fetch_assoc($resultData);
        return($ans);
